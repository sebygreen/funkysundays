    ...
    if (options && options.expanded) {
        ...
        if (data.expand) {
            if (data.expand.partners) {
                this.partners = data.expand.partners.map(
                    (i) => new Collaboration(i),
                );
            }
            if (data.expand.sponsors) {
                this.sponsors = data.expand.sponsors.map(
                    (i) => new Collaboration(i),
                );
            }
            if (data.expand["schedule(event)"]) {
                this.schedule = data.expand["schedule(event)"].map(
                    (i) => new Set(i),
                );
            }
        }
...
export function Artist(data, options) {
    ...
    if (options && options.expanded) {
        ...
        this.links = links;
        let upcoming = data.expand["schedule(artist)"].find((i) =>
            dayjs(i.start).isAfter(dayjs()),
        );
        if (upcoming) {
